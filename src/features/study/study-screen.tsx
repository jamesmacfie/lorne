...
import { BookOpenCheck, Eye, Lightbulb, RotateCcw, WifiOff } from "lucide-react";
import type { ReviewRating, StudyCard } from "#/shared/contracts";
...
export function StudyScreen({ userId }: { userId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setQueue(data.queue);
      setSessionGoal(Math.min(10, data.queue.length));
      await cacheStudyQueue(userId, data.queue);
    } catch {
      const cached = await getCachedStudyQueue(userId);
      if (cached.length) {
        setQueue(cached);
    ...
    <main className="page page--study">
      ...
      {!online && (
        <p className="offline-note">
          <WifiOff aria-hidden /> Saved here first. Lorne will sync when you reconnect.
        </p>
      )}
    </main>
  );
